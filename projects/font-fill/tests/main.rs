use fill_fonts::{FontFillCanvas, FontFillVideo};
...
fn main() {
    ...
    let mut file = FontFillVideo::create("../../target/SourceHanSansSC", font, 144).unwrap();
    for c in include_str!("长恨歌.txt").chars() {
        if !c.is_alphabetic() {
            continue;
        }
        file.encode_frame(c, Rgba([1.0, 0.0, 0.0, 1.0]));
    }
